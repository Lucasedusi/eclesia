...
import {normalizePublicCaravanPaymentInput} from "@/modules/events/utils/payment-payload";
...
export async function POST(request:NextRequest,{params}:{params:Promise<{publicCode:string;slug:string}>}){
  ...
  try{
    ...
    const parsed=publicCaravanSchema.safeParse(normalizePublicCaravanPaymentInput(payload));if(!parsed.success)return NextResponse.json({message:parsed.error.issues[0]?.message??"Revise os dados da caravana.",fieldErrors:parsed.error.flatten().fieldErrors},{status:400});
