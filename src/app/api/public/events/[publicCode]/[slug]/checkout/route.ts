...
import { consumePublicMemberLinkRateLimit, consumePublicRegistrationRateLimit, getPublicEvent } from "@/modules/events/services/event.service";
...
export async function POST(request: NextRequest, context: { params: Promise<{ publicCode: string; slug: string }> }) {
  ...
  if (Number(request.headers.get("content-length") ?? 0) > MAX_BODY_SIZE) return NextResponse.json({ message: "Solicitação muito grande." }, { status: 413 });
  ...
  try {
    ...
    if (typeof body.website === "string" && body.website.trim()) return NextResponse.json({ message: "Inscrição recebida." }, { status: 201 });
    ...
    if (!publicEvent || publicEvent.event.id !== parsed.data.eventId) return NextResponse.json({ message: "Evento indisponível para inscrições." }, { status: 404 });
    ...
    if (!replay) {
      if (!publicEvent.isRegistrationOpen) return NextResponse.json({ message: "Evento indisponível para inscrições." }, { status: 404 });
    ...
    const message = error instanceof Error ? error.message : "Não foi possível iniciar a inscrição.";
