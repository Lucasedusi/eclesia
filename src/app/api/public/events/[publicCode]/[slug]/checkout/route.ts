...
import { consumePublicRegistrationRateLimit, getPublicEvent } from "@/modules/events/services/event.service";
...
export async function POST(request: NextRequest, context: { params: Promise<{ publicCode: string; slug: string }> }) {
  ...
  try {
    ...
    if (!publicEvent || publicEvent.event.id !== parsed.data.eventId || !publicEvent.isRegistrationOpen) return NextResponse.json({ message: "Evento indisponível para inscrições." }, { status: 404 });
