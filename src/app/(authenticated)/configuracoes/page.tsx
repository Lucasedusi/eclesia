...
import { ChurchCredentialLogoForm } from "@/modules/auth/components/church-settings/church-credential-logo-form";
...
export default async function SettingsPage() {
  ...
  const [church, settings] = await Promise.all([
    supabase.from("churches").select("name, legal_name, document, email, phone, whatsapp, senior_pastor_name, senior_pastor_spouse_name, logo_url").eq("id",context.church.id).single(),
    supabase.from("app_settings").select("display_church_name, member_code_prefix, member_code_padding, logo_url").eq("church_id",context.church.id).is("deleted_at",null).single(),
  ]);
  const logoDataUri = await loadChurchLogoDataUri(settings.data?.logo_url || church.data?.logo_url || null, context.church.id, supabase);
  return <>
    ...
    <ChurchCredentialLogoForm logoDataUri={logoDataUri} canUpdate={hasPermission(context.permissions, PERMISSIONS.settingsUpdate)} />
  </>;
}
