...
export default async function AuditPage() {
  ...
  return <>
    ...
    <section className="overflow-hidden rounded-[20px] border border-[#EAECF0] bg-white shadow-[var(--shadow-card)]">
      <div className="grid grid-cols-[1.05fr_.7fr_.8fr_1.3fr_.7fr] gap-4 border-b border-[#EAECF0] bg-[#F9FAFB] px-5 py-3 text-[10px] font-bold uppercase tracking-wide text-slate-400 max-lg:hidden">
      ...
      {logs.length ? <div className="divide-y divide-[#EAECF0]">{logs.map((log)=><article key={log.id} className="grid grid-cols-[1.05fr_.7fr_.8fr_1.3fr_.7fr] items-center gap-4 px-5 py-4 max-lg:grid-cols-1 max-lg:gap-2">
        ...
        <div><strong className="block text-[11px] font-bold text-slate-700">{log.module}</strong><span className="mt-1 block text-[9px] font-semibold text-slate-400">{log.action}</span></div>
        <span className="text-[11px] font-semibold text-slate-500">{log.entity}</span>
        <p className="m-0 text-[11px] font-normal leading-5 text-slate-500">{log.description}</p>
        <time className="text-[10px] font-semibold text-slate-400">{dateFormatter.format(new Date(log.createdAt))}</time>
