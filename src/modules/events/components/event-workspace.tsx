...
export function EventWorkspace({ initial }: { initial: EventWorkspaceData }) {
  ...
  const [caravanSort,setCaravanSort]=useState<CaravanSortOrder>("NEWEST");
  ...
  const filteredCaravans=useMemo(()=>sortCaravans(initial.groups.filter((group)=>{
    ...
    return true;
  }),caravanSort),[caravanCity,caravanFrom,caravanItem,caravanList,caravanPayment,caravanPaymentMethod,caravanSearch,caravanSort,caravanSource,caravanState,caravanStatus,caravanTo,initial.groups,initial.payments]);
  const hasCaravanFilters=Boolean(caravanSearch||caravanCity||caravanState||caravanStatus||caravanPayment||caravanPaymentMethod||caravanSource||caravanList||caravanItem||caravanFrom||caravanTo||caravanSort!=="NEWEST");
    ...
    <S.Module>
      ...
      {active === "registrations" ? (
        <Section title="Inscrições" description={event.registrationStatus === "OPEN" ? undefined : "As inscrições estão fechadas. Abra-as no cabeçalho para permitir novos cadastros."} action={can(PERMISSIONS.eventRegistrationsManage) && event.registrationStatus === "OPEN" ? <Button onClick={() => setModal("registration")}><Plus size={15} />Nova inscrição</Button> : null}>
          ...
          {filteredRegistrations.length ? <S.TableWrap><table><thead><tr><th>Participante</th><th>Telefone</th><th>Regional / congregação</th><th>Itens</th><th>Valor</th><th>Pagamento</th><th>Situação</th><th>Ações</th></tr></thead><tbody>
      ...
      {active === "groups" ? (
        <Section title="Caravanas" description={event.registrationStatus === "OPEN" ? "Inscrições coletivas independentes, com quantidades, itens e pagamentos próprios." : "As inscrições estão fechadas. Abra-as para cadastrar novas caravanas."} action={can(PERMISSIONS.eventGroupsManage) && event.registrationStatus === "OPEN" ? <Button onClick={() => setModal("group")}><Plus size={15} />Nova caravana</Button> : null}>
          <S.PrimaryCaravanFilters>
            ...
            <S.Field><span>Ordenar por</span><select value={caravanSort} onChange={(change)=>setCaravanSort(change.target.value as CaravanSortOrder)}><option value="NEWEST">Mais recentes</option><option value="MOST_REGISTRATIONS">Mais inscrições</option><option value="LEAST_REGISTRATIONS">Menos inscrições</option><option value="ALPHABETICAL_ASC">Ordem alfabética A–Z</option><option value="ALPHABETICAL_DESC">Ordem alfabética Z–A</option></select></S.Field>
            <S.FilterActions>
          ...
          {filteredCaravans.length ? <S.TableWrap><table><thead><tr><th>Caravana / origem</th><th>Responsável</th><th>Participantes</th><th>Total / recebido</th><th>Pagamento</th><th>Situação</th><th>Ações</th></tr></thead><tbody>{filteredCaravans.map((group) => <tr key={group.id}>
