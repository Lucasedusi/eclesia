...
import { isPublicManualPaymentMethod, PUBLIC_BACK_LABEL, publicRefreshNotice, publicTrackingState, readTrackingHash, shouldPollPublicStatus, shouldShowTrackingPix, type PublicTrackingKind } from "../utils/public-registration-flow";
import { QrCode } from "./qr-code";
import { PublicEventHeroMeta, PublicFlowToast, type PublicFlowNotice } from "./public-event-mobile";
...
export function PublicRegistrationTracking({ event }: { event: EventDetail }) {
  ...
  function refresh() {
    if (!reference) return;
    startTransition(async () => {
      try { const next = await queryStatus(reference, true); setNotice(publicRefreshNotice(next.status)); }
  ...
  const individualWhatsappNumber = (event.paymentSettings.individual.whatsappNumber ?? "").replace(/\D/g, "");
  const caravanWhatsappNumber = (event.paymentSettings.whatsappNumber ?? "").replace(/\D/g, "");
  ...
  return <S.PublicShell><S.PublicCard>
    ...
    <S.PublicTrackingContent>
      ...
      {reference === null && !tracking ? <><S.PublicBackRow aria-label="Navegação de retorno"><S.PublicBackLink as={Link} href={`${newRegistrationHref}?nova=1`}><ArrowLeft/>{PUBLIC_BACK_LABEL}</S.PublicBackLink></S.PublicBackRow><S.PublicTrackingEmpty><span><TriangleAlert/></span><h2>Link de acompanhamento inválido</h2><p>Abra novamente o link recebido ao concluir a inscrição ou faça uma nova inscrição.</p></S.PublicTrackingEmpty></> : null}
      {tracking?.kind === "INDIVIDUAL" ? <IndividualTracking status={tracking.status} data={tracking.data} whatsappNumber={individualWhatsappNumber} onRefresh={refresh} onDownload={downloadIndividualReceipt} onNotice={(message)=>setNotice({message})} pending={pending} resumeHref={`${newRegistrationHref}?retomar=1`} /> : null}
      {tracking?.kind === "CARAVAN" ? <CaravanTracking status={tracking.status} data={tracking.data} token={reference?.token ?? ""} publicCode={event.publicCode ?? ""} slug={event.slug ?? ""} whatsappNumber={caravanWhatsappNumber} onRefresh={refresh} pending={pending} /> : null}
    </S.PublicTrackingContent>
    <PublicFlowToast notice={notice} onClose={()=>setNotice(null)}/>
  </S.PublicCard></S.PublicShell>;
}

function IndividualTracking({ status, data, whatsappNumber, onRefresh, onDownload, onNotice, pending, resumeHref }: {
  status: string; data: PublicCheckoutStatus; whatsappNumber: string; onRefresh: () => void; onDownload: (data: PublicCheckoutStatus) => void; onNotice: (message: string) => void; pending: boolean; resumeHref: string;
}) {
  ...
  return <S.PublicTrackingPanel>
    <S.PublicTrackingHeader data-pending={state.pending} data-danger={state.failed}><span>{complete?<CheckCircle2/>:state.failed?<TriangleAlert/>:<Clock3/>}</span><div><S.PublicStatusBadge $tone={state.tone}>{registrationStatus(status)}</S.PublicStatusBadge><h2>{complete?"Inscrição confirmada":state.failed?"Inscrição indisponível":"Inscrição recebida"}</h2><p>{complete?"A credencial e o comprovante já estão disponíveis.":state.failed?"Esta inscrição não está mais ativa. Consulte a situação indicada acima.":isPublicManualPaymentMethod(data.paymentMethod)?`Entre em contato pelo WhatsApp para combinar o pagamento ${data.paymentMethod === "CASH" ? "em dinheiro" : "presencial"}.`:"Aguardando a confirmação do pagamento."}</p></div></S.PublicTrackingHeader>
    ...
    {data.credentialToken?<S.PublicTrackingCredential><div><small>CREDENCIAL DO EVENTO</small><h3>{data.participantName}</h3><p>{data.registrationNumber}{data.congregationName?` · ${data.congregationName}`:""}</p></div><QrCode value={data.credentialToken} size={150}/></S.PublicTrackingCredential>:<S.PublicTrackingCredentialPending $danger={state.failed} role={state.failed?"alert":"status"}><span>{state.failed?<TriangleAlert/>:<LockKeyhole/>}</span><div><small>CREDENCIAL DO EVENTO</small><h3>{state.failed?"Credencial indisponível":"Credencial aguardando liberação"}</h3><p>{state.failed?"A credencial não pode ser liberada para a situação atual da inscrição.":"Ela aparecerá automaticamente depois da confirmação da inscrição."}</p></div></S.PublicTrackingCredentialPending>}
    <S.PublicTrackingActions>{whatsapp?<S.PublicWhatsappLink href={whatsapp} target="_blank" rel="noreferrer"><MessageCircle/>Combinar pelo WhatsApp</S.PublicWhatsappLink>:null}{!complete&&!state.failed&&data.paymentMethod==="PIX"&&!data.providerPaymentId&&!data.receiptSubmitted?<Link className="app-button-secondary" href={resumeHref}><CreditCard/>Continuar pagamento</Link>:null}<Button variant="outline" onClick={onRefresh} loading={pending}><RefreshCw/>Atualizar situação</Button>{complete?<><Button variant="outline" onClick={()=>window.print()}><Printer/>Imprimir</Button><Button onClick={()=>onDownload(data)} loading={pending}><Download/>Baixar comprovante</Button></>:null}</S.PublicTrackingActions>
  </S.PublicTrackingPanel>;
}

function CaravanTracking({ status, data, token, publicCode, slug, whatsappNumber, onRefresh, pending }: {
  status: string; data: Extract<PublicTrackingStatus,{kind:"CARAVAN"}>["data"]; token: string; publicCode: string; slug: string; whatsappNumber: string; onRefresh: () => void; pending: boolean;
}) {
  const state = publicTrackingState(status);
  const complete = state.complete;
  const whatsapp = !complete && !state.failed && data.paymentMethod === "CASH" && whatsappNumber ? `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Olá! Gostaria de combinar o pagamento em dinheiro da caravana ${data.groupNumber} — ${data.eventName}. Responsável: ${data.responsibleName}. Origem: ${data.originChurchName}. Total: ${money(data.totalAmount)}.`)}` : "";
  return <S.PublicTrackingPanel>
    <S.PublicTrackingHeader data-pending={state.pending} data-danger={state.failed}><span>{complete?<CheckCircle2/>:state.failed?<TriangleAlert/>:<Bus/>}</span><div><S.PublicStatusBadge $tone={state.tone}>{registrationStatus(status)}</S.PublicStatusBadge><h2>{state.failed?"Caravana indisponível":`Caravana ${data.groupNumber}`}</h2><p>{complete?"Pagamento confirmado e comprovante disponível.":state.failed?"Esta inscrição não está mais ativa. Consulte a situação indicada acima.":data.paymentMethod==="CASH"?"Use o WhatsApp para combinar o pagamento em dinheiro com a organização.":"A inscrição foi recebida e aguarda a conferência do pagamento."}</p></div></S.PublicTrackingHeader>
