...
export function PublicRegistrationTracking({ event }: { event: EventDetail }) {
  ...
  return <S.PublicShell><S.PublicCard>
    ...
    <S.PublicTrackingContent>
      ...
      {tracking?.kind === "INDIVIDUAL" ? <IndividualTracking status={tracking.status} data={tracking.data} whatsappNumber={whatsappNumber} onRefresh={refresh} onDownload={downloadIndividualReceipt} onNotice={(message)=>setNotice({message})} pending={pending} resumeHref={`${newRegistrationHref}?retomar=1`} /> : null}
      {tracking?.kind === "CARAVAN" ? <CaravanTracking status={tracking.status} data={tracking.data} token={reference?.token ?? ""} publicCode={event.publicCode ?? ""} slug={event.slug ?? ""} whatsappNumber={whatsappNumber} onRefresh={refresh} pending={pending} /> : null}
    </S.PublicTrackingContent>
