...
import { ArrowLeft, ArrowRight, Banknote, BedDouble, Bus, CalendarDays, Check, CheckCircle2, Clock3, Copy, CreditCard, Download, Gift, LoaderCircle, LockKeyhole, MapPin, MessageCircle, Minus, Plus, Printer, QrCode as QrCodeIcon, ShieldCheck, Shirt, Smartphone, Ticket, UtensilsCrossed } from "lucide-react";
...
type Role = { id: string; name: string; femaleName: string | null };
type Props = { event: EventDetail; items: Item[]; congregations: Congregation[]; roles: Role[]; fields: EventRegistrationFieldRow[]; isRegistrationOpen: boolean };
type Step = 1 | 2 | 3;
...
function PublicCustomField({ field, values, onChange }: { field: EventRegistrationFieldRow; values: Record<string, unknown>; onChange: (next: Record<string, unknown>) => void }) {
...
export function PublicRegistration({ event, items, congregations, roles, fields, isRegistrationOpen }: Props) {
  ...
  function renderRegistrationField(field: EventRegistrationFieldRow) {
    ...
    if (field.kind === "CUSTOM") return <PublicCustomField field={field} values={customValues} onChange={setCustomValues} />;
    if (field.key === "participant_name") return <S.Field><span>{label}</span><input name="participantName" autoComplete="name" required={isRequired} minLength={3} aria-invalid={Boolean(fieldErrors.participantName)} />{fieldErrors.participantName?.map((error) => <S.ErrorText key={error}>{error}</S.ErrorText>)}</S.Field>;
    if (field.key === "participant_role_id") return <S.Field><span>{label}</span><select name="participantRoleId" required={isRequired} defaultValue=""><option value="">Sem cargo informado</option>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select>{fieldErrors.participantRoleId?.map((error) => <S.ErrorText key={error}>{error}</S.ErrorText>)}</S.Field>;
    if (field.key === "participant_gender") return <S.Field><span>{label}</span><select name="participantGender" required={isRequired} aria-invalid={Boolean(fieldErrors.participantGender)} defaultValue=""><option value="">Selecione uma opção</option><option value="MALE">Masculino</option><option value="FEMALE">Feminino</option></select>{fieldErrors.participantGender?.map((error) => <S.ErrorText key={error}>{error}</S.ErrorText>)}</S.Field>;
    if (field.key === "participant_phone") return <S.Field><span>{label}</span><input name="participantPhone" autoComplete="tel" inputMode="tel" required={isRequired} minLength={isRequired ? 8 : undefined} aria-invalid={Boolean(fieldErrors.participantPhone)} placeholder="(00) 00000-0000" onChange={(change) => { change.currentTarget.value = formatBrazilPhone(change.currentTarget.value); }} />{fieldErrors.participantPhone?.map((error) => <S.ErrorText key={error}>{error}</S.ErrorText>)}</S.Field>;
    if (field.key === "participant_email") return <S.Field><span>{label}</span><input name="participantEmail" type="email" required={isRequired} /></S.Field>;
    if (field.key === "participant_document") return <S.Field><span>{label}</span><input name="participantDocument" required={isRequired} /></S.Field>;
    if (field.key === "participant_birth_date") return <S.Field><span>{label}</span><input name="participantBirthDate" type="date" required={isRequired} /></S.Field>;
    if (field.key === "region_id") return <S.Field><span>{label}</span><select required={isRequired} value={regionId} onChange={(change) => setRegionId(change.target.value)}><option value="">Sem regional</option>{regions.map((region) => <option key={region.id} value={region.id}>{region.name}</option>)}</select></S.Field>;
    if (field.key === "congregation_id") return <S.Field><span>{label}</span><select name="congregationId" required={isRequired} defaultValue=""><option value="">Sem congregação</option>{filteredCongregations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></S.Field>;
    if (field.key === "participant_city") return <S.Field><span>{label}</span><input name="participantCity" required={isRequired} /></S.Field>;
    if (field.key === "participant_state") return <S.Field><span>{label}</span><input name="participantState" maxLength={2} required={isRequired} /></S.Field>;
    if (field.key === "responsible_name") return <S.Field><span>{label}</span><input name="responsibleName" required={isRequired} /></S.Field>;
    if (field.key === "responsible_phone") return <S.Field><span>{label}</span><input name="responsiblePhone" required={isRequired} onChange={(change) => { change.currentTarget.value = formatBrazilPhone(change.currentTarget.value); }} /></S.Field>;
    if (field.key === "items" && items.length) return <S.Wide><fieldset><S.FieldLegend>{field.label}</S.FieldLegend><S.ItemInstruction>Clique sobre os itens que deseja adicionar à inscrição.</S.ItemInstruction><S.PublicItemCards>{items.map((item) => { const Icon = itemIcons[item.item_type] ?? Ticket; const checked = selected.has(item.id); const soldOut = item.available_quantity === 0; return <S.PublicItemCard key={item.id} $selected={checked} data-disabled={soldOut}><input type="checkbox" checked={checked} disabled={item.is_required || soldOut} onChange={() => setSelected((current) => { const next = new Set(current); if (next.has(item.id) && !item.is_required) next.delete(item.id); else next.add(item.id); return next; })} aria-label={`Selecionar ${item.name}`} /><span><Icon /></span><div><strong>{item.name}</strong>{item.description ? <p>{item.description}</p> : null}<small>{soldOut ? "Esgotado" : `${money(item.price)}${item.is_required ? " · obrigatório" : ""}`}</small></div>{checked ? <CheckCircle2 /> : null}{checked && item.allow_quantity ? <S.QuantityControl><button type="button" onClick={(click) => { click.preventDefault(); changeQuantity(item, -1); }} aria-label={`Diminuir ${item.name}`}><Minus /></button><b>{quantities[item.id] ?? 1}</b><button type="button" onClick={(click) => { click.preventDefault(); changeQuantity(item, 1); }} aria-label={`Aumentar ${item.name}`}><Plus /></button></S.QuantityControl> : null}</S.PublicItemCard>; })}</S.PublicItemCards></fieldset></S.Wide>;
    if (field.key === "preferred_payment_method" && selectedTotal > 0) return <S.Wide><fieldset><S.FieldLegend>{field.label}</S.FieldLegend><S.PaymentChoices>{methodOptions.map((method) => { const Icon = method.icon; return <label key={method.value} data-selected={paymentMethod === method.value}><input type="radio" name="paymentMethod" value={method.value} checked={paymentMethod === method.value} onChange={() => setPaymentMethod(method.value)} /><span><Icon /></span><div><strong>{method.label}</strong><small>{method.description}</small></div>{paymentMethod === method.value ? <CheckCircle2 /> : null}</label>; })}</S.PaymentChoices></fieldset></S.Wide>;
    return null;
  }
  ...
  function applyCheckoutStatus(status:PublicCheckoutStatus){
    setCheckout(status);
    if(status.items.length){setSelected(new Set(status.items.map((item)=>item.id)));setQuantities((current)=>({...current,...Object.fromEntries(status.items.map((item)=>[item.id,item.quantity]))}));}
  ...
  function submitRegistration(formEvent: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try{
        ...
        const data = body.data as { checkoutToken: string;checkout?:PublicCheckoutStatus }; setCheckoutToken(data.checkoutToken); sessionStorage.setItem(checkoutKey, data.checkoutToken);
        const status=data.checkout??await queryStatus(data.checkoutToken,false);if(data.checkout)applyCheckoutStatus(data.checkout);
        setStep(status.totalAmount <= 0 ? 3 : 2); window.scrollTo({ top: 0, behavior: "smooth" });
      }catch(error){setNotice({message:error instanceof Error?error.message:"A inscrição foi recebida, mas não foi possível abrir a próxima etapa. Recarregue a página para continuar.",danger:true});}
    });
  }
  ...
  const cashWhatsapp = checkout && activeMethod === "CASH" && whatsappNumber
    ? `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Olá! Gostaria de combinar o pagamento em dinheiro da inscrição ${checkout.registrationNumber} de ${checkout.participantName} para o evento ${event.name}. Total: ${money(checkout.totalAmount)}.`)}`
    : "";

  return <S.PublicShell><S.PublicCard>
    <S.PublicHero $image={event.bannerUrl}><div><span>{isRegistrationOpen ? "Inscrições abertas" : "Inscrições encerradas"}</span><h1>{event.name}</h1><p>{event.description || "Participe deste evento."}</p></div></S.PublicHero>
    ...
    <S.PublicContent><section>
      ...
      {step === 2 ? <S.CheckoutPanel>
        ...
        {activeMethod === "PIX" ? !pix ? <form onSubmit={generatePix}>
          ...
          {pix.qrCode ? <S.PixCopy><code>{pix.qrCode}</code><Button type="button" size="sm" onClick={copyPix} variant="outline"><Copy size={14} />{pix.isSimulated ? "Copiar código de teste" : "Copiar código Pix"}</Button></S.PixCopy> : null}
        ...
        </S.PixCodePanel></S.PixLayout> : <S.ManualPayment><span><Banknote /></span><h3>Inscrição recebida</h3><p>Você escolheu pagar com <strong>{methodLabel(activeMethod)}</strong>. O pagamento será recebido e confirmado pela organização do evento.</p><S.ManualPaymentHint><LockKeyhole /><span>A credencial será liberada após a confirmação do pagamento.</span></S.ManualPaymentHint>{cashWhatsapp?<S.PublicWhatsappLink href={cashWhatsapp} target="_blank" rel="noreferrer"><MessageCircle/>Falar pelo WhatsApp</S.PublicWhatsappLink>:null}<Button onClick={() => setStep(3)}>Ver protocolo da inscrição<ArrowRight size={16} /></Button></S.ManualPayment>}
      </S.CheckoutPanel> : null}
