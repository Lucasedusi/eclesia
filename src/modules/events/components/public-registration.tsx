...
export function PublicRegistration({ event, items, congregations, roles, fields, isRegistrationOpen }: Props) {
  ...
  }, [checkoutKey, trackingPath]);

  useEffect(() => {
    if (!checkoutToken || checkout?.registrationStatus !== "CONFIRMED") return;
    window.location.replace(`${trackingPath}${buildTrackingHash("INDIVIDUAL", checkoutToken)}`);
  }, [checkout?.registrationStatus, checkoutToken, trackingPath]);
    ...
    <S.PublicContent><section>
      ...
      {step === 2 ? <S.CheckoutPanel>
        ...
        {activeMethod === "PIX" ? !pix ? <form onSubmit={generatePix}>
          ...
          <S.PixStatus><span><Clock3 /></span><div><small>SITUAÇÃO</small><strong>{pix.isSimulated ? "Aguardando aprovação de teste" : pixReady ? "Aguardando pagamento" : "Preparando QR Code"}</strong></div><time>{timerLabel}</time></S.PixStatus>
