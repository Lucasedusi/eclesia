    ...
    <S.Module>
      ...
      <S.Panel>
        ...
        {pagedItems.length === 0 ? (
          ...
          <>
            {activeTab === "regions" && <RegionTable items={pagedItems as RegionItem[]} canManage={canManage} busyId={busyId} onEdit={setRegionForm} onStatus={(item) => changeStatus("region", item.id, item.status)} onDelete={setDeleteTarget} />}
            {activeTab === "congregations" && <CongregationTable items={pagedItems as CongregationItem[]} canManage={canManage} busyId={busyId} onEdit={setCongregationForm} onStatus={(item) => changeStatus("congregation", item.id, item.status)} onDelete={setDeleteTarget} />}
...
type CommonTableProps<T> = { items: T[]; canManage: boolean; busyId: string | null; onEdit: (item: T) => void; onStatus: (item: T) => void; onDelete: (target: DeleteTarget) => void };

function Actions({ item, canManage, busy, protectedItem = false, onEdit, onStatus, onDelete }: { item: RegionItem | CongregationItem | PositionItem; kind?: DeleteTarget["kind"]; canManage: boolean; busy: boolean; protectedItem?: boolean; onEdit: () => void; onStatus: () => void; onDelete: () => void }) {
  if (!canManage) return null;
  return <S.RowActions>
    <S.IconButton type="button" title="Editar" aria-label={`Editar ${item.name}`} onClick={onEdit} disabled={busy}><Pencil /></S.IconButton>
    {!protectedItem ? <S.IconButton type="button" $warning title={item.status === "ACTIVE" ? "Inativar" : "Ativar"} aria-label={`${item.status === "ACTIVE" ? "Inativar" : "Ativar"} ${item.name}`} onClick={onStatus} disabled={busy}>{busy ? <S.InlineSpinner /> : <Power />}</S.IconButton> : null}
    {!protectedItem ? <S.IconButton type="button" $danger title="Arquivar" aria-label={`Arquivar ${item.name}`} onClick={onDelete} disabled={busy}><Trash2 /></S.IconButton> : null}
  </S.RowActions>;
}

function RegionTable({ items, canManage, busyId, onEdit, onStatus, onDelete }: CommonTableProps<RegionItem>) {
  return <><S.TableWrap><S.Table><thead><tr><th>Regional</th><th>Coordenação</th><th>Congregações</th><th>Ordem</th><th>Status</th><th>Atualização</th>{canManage && <th aria-label="Ações" />}</tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><S.PrimaryCell><strong><Network size={14} />{item.name}</strong><small>{item.description || "Sem descrição"}</small></S.PrimaryCell></td><td>{item.coordinatorName || "Não informado"}<br /><small>{item.coordinatorPhone || ""}</small></td><td>{item.congregationCount} total · {item.activeCongregationCount} ativa{item.activeCongregationCount === 1 ? "" : "s"}</td><td>{item.displayOrder}</td><td><Status value={item.status} /></td><td>{formatDate(item.updatedAt)}</td>{canManage && <td><Actions item={item} kind="region" canManage busy={busyId === item.id} onEdit={() => onEdit(item)} onStatus={() => onStatus(item)} onDelete={() => onDelete({ kind: "region", id: item.id, name: item.name, details: "A regional só pode ser arquivada quando não houver congregações vinculadas." })} /></td>}</tr>)}</tbody></S.Table></S.TableWrap><MobileRows items={items} canManage={canManage} busyId={busyId} kind="region" onEdit={onEdit} onStatus={onStatus} onDelete={onDelete} /></>;
}

function CongregationTable({ items, canManage, busyId, onEdit, onStatus, onDelete }: CommonTableProps<CongregationItem>) {
  return <><S.TableWrap><S.Table><thead><tr><th>Congregação</th><th>Regional</th><th>Responsável</th><th>Localidade</th><th>Status</th><th>Atualização</th>{canManage && <th aria-label="Ações" />}</tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><S.PrimaryCell><strong><Church size={14} />{item.name}{item.isHeadquarters && <S.HeadquartersBadge>Sede</S.HeadquartersBadge>}</strong><small>{item.code || item.email || "Sem código"}</small></S.PrimaryCell></td><td>{item.regionName || "Sem regional"}</td><td>{item.pastorName || "Não informado"}</td><td>{[item.city, item.state].filter(Boolean).join(" / ") || "Não informada"}</td><td><Status value={item.status} /></td><td>{formatDate(item.updatedAt)}</td>{canManage && <td><Actions item={item} kind="congregation" canManage busy={busyId === item.id} protectedItem={item.isHeadquarters} onEdit={() => onEdit(item)} onStatus={() => onStatus(item)} onDelete={() => onDelete({ kind: "congregation", id: item.id, name: item.name, details: "O arquivamento é bloqueado quando existem membros, usuários ou outros vínculos dependentes." })} /></td>}</tr>)}</tbody></S.Table></S.TableWrap><MobileRows items={items} canManage={canManage} busyId={busyId} kind="congregation" onEdit={onEdit} onStatus={onStatus} onDelete={onDelete} /></>;
}
...
function MobileRows<T extends RegionItem | CongregationItem | PositionItem>({ items, canManage, busyId, kind, onEdit, onStatus, onDelete }: CommonTableProps<T> & { kind: DeleteTarget["kind"] }) {
